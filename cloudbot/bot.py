...
class CloudBot:
    ...
    def run(self):
        ...
        # Wait till the bot stops. The stopped_future will be set to True to
    ...
    def create_connections(self):
        ...
        for config in self.config['connections']:
            ...
            local_bind = (config['connection'].get('bind_addr', False), config[
                'connection'].get('bind_port', 0))
            if local_bind[0] is False:
                local_bind = False

            self.connections[name] = IrcClient(self, name, nick, config=config, channels=config['channels'],
                                               server=server, port=port, use_ssl=config[
                    'connection'].get('ssl', False),
    ...
    def _init_routine(self):
        ...
        # Run a manual garbage collection cycle, to clean up any unused objects
    ...
    def process(self, event):
        ...
        for raw_hook in self.plugin_manager.catch_all_triggers:
            # run catch-all coroutine hooks before all others - TODO: Make this
            # a plugin argument
