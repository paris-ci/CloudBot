...
def yahoo_finance_xchange(text):
    ...
    try:
        encoded = urlencode({'q'     : query, 'env': 'store://datatables.org/alltableswithkeys',
                             'format': 'json'})  # URL encode the query, as well as other needed variables
    ...
    if int(data['query']['count']) > 1:  # If the user requests multiple conversions, the results are provided as a list
        for item in data['query']['results']['rate']:
            if item['Name'] != 'N/A':
                rates.append([item['Name'].split('/').pop(), float(item[
                                                                       'Rate'])])  # Since the Name is written as USD/DKK, and since we are only interested in the target currency, we split by / and pop off the last element

    else:  # If only one target currency is supplied, we don't get a list, just the result
        if data['query']['results']['rate']['Rate'] != 'N/A':
            rates.append([data['query']['results']['rate']['Name'].split('/').pop(),
                          float(data['query']['results']['rate']['Rate'])])
    ...
    for item in rates:
        converted.append([float(text_list[0]) * float(item[1]), item[
            0]])  # Save a list with the values converted according to the rate, and the name of the currency

    user_string = '{} {} is '.format(text_list[0], text_list[
        1])  # Prepare the userstring. We already know the original currency, and the amount

    for (i, item) in enumerate(
            converted):  # We add the converted values in a for loop, to accomodate several target currencies
        if i > 0:  # If the user has converted to multiple currencies, add a nice or in between
            user_string += ' or '

        user_string += '{:,.2f} {}'.format(float(item[0]),
                                           item[1])  # Add the formated converted result to the user string
